...
add_util_methods = (Util)->
  ...
  Util.file_to_array = (filename,options)->
    ...
    a = a.split("\n")                                      # split by newlines
    a = a.map (e)=>Util.trim(e) if options.trim?           # trim leading and trailing whilespace if needed
    predicate = null                                       # create a predicate for filtering if needed
    re = new RegExp("^\s*#{options.comment_char}")
  ...
  Util.sort_by_value = (m)->
    return (map_to_array(m)).sort (a,b)-> (a[1] - b[1])

  # `sort_by_key` - return an array of name/value pairs, ordred by name.
  Util.sort_by_key = (m)->
    return (map_to_array(m)).sort (a,b)-> (a[0] - b[0])
